...
model = PadimWithLogging(backbone="resnet18")
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        trainer = Trainer(max_epochs=110,
    ...
    trainer.fit(model=model, datamodule=datamodule,ckpt_path=r"tb_logs\padim_training\version_4\checkpoints\epoch=99-step=200.ckpt")

    import torch._dynamo
    torch._dynamo.disable()
    model.to_onnx("Models")
    # gc.collect()
    # torch.cuda.empty_cache()
